...
import { LawyerRequest } from '../../types';

interface LawyerTrashModalProps {
    isOpen: boolean;
    onClose: () => void;
    session: any;
    onRestore: () => void;
...
export const LawyerTrashModal: React.FC<LawyerTrashModalProps> = ({ isOpen, onClose, session, onRestore }) => {
    const [deletedRequests, setDeletedRequests] = useState<LawyerRequest[]>([]);
    ...
    const fetchDeletedRequests = async () => {
        ...
        if (error) {
            ...
            setDeletedRequests(data.map((d: any) => ({
                id: d.id,
                name: d.name,
    ...
    const handleDeletePermanently = async (id: string) => {
        if (!confirm('ATENÇÃO: Isso excluirá o requerimento permanentemente. Deseja continuar?')) return;
            ...
            <div className="bg-white rounded-xl shadow-2xl w-full max-w-4xl h-[80vh] flex flex-col animate-in fade-in zoom-in-95 duration-200">
                <div className="bg-red-50 border-b border-red-100 p-4 flex justify-between items-center">
                    <h3 className="font-bold text-lg text-red-800 flex items-center gap-2">
                        <Trash2 size={20} /> Lixeira Advogados
                    </h3>
                    ...
                    {loading ? (
                        ...
                        <div className="grid gap-3">
                            {filteredRequests.map(req => (
                                <div key={req.id} className="bg-white p-4 rounded-lg border border-gray-200 hover:shadow-md transition-shadow flex justify-between items-center group">
                                    <div>
                                        <h4 className="font-bold text-gray-800">{req.name}</h4>
                                        <div className="text-sm text-gray-500 mt-1 flex gap-3 flex-wrap">
                                            {req.caseNumber && <span className="bg-gray-100 px-2 py-0.5 rounded text-xs font-mono">{req.caseNumber}</span>}
                                            <span className="bg-blue-50 text-blue-700 px-2 py-0.5 rounded text-xs uppercase">{req.contactMethod}</span>
                                            <span className="bg-purple-50 text-purple-700 px-2 py-0.5 rounded text-xs uppercase">{req.matter}</span>
                                        </div>
                                        <p className="text-xs text-red-400 mt-2">
                                            Excluído em: {new Date(req.deletedAt!).toLocaleString()}
                                        </p>
                                    </div>
                                    <div className="flex gap-2">
                                        ...
                                        <button
                                            onClick={() => handleDeletePermanently(req.id)}
